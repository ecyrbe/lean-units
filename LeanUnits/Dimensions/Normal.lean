...
theorem exponentOf_merge (l₁ l₂ : Bases) (h₁ : Sorted l₁) (h₂ : Sorted l₂) (name : String) :
  ...
  -- all cases
  -- given that : ∃ b : Base, (b ∈ l ∧ b.name = name) → ExponentOf name l = b.exponent
  -- and that : (¬ ∃ b : Base, b ∈ l ∧ b.name = name) → ExponentOf name l = 0
  -- then for l₁ and l₂ we have 4 cases :
  -- ∃ b : Base, (b ∈ l₁ ∧ b.name = name) ∧ (∀ b' ∈ l₂, b'.name ≠ name) → ExponentOf name l₁ = b.exponent ∧ ExponentOf name l₂ = 0
  -- ∃ b : Base, (b ∈ l₂ ∧ b.name = name) ∧ (∀ b' ∈ l₁, b'.name ≠ name) → ExponentOf name l₁ = 0 ∧ ExponentOf name l₂ = b.exponent
  -- ∃ b, b' : Base, (b ∈ l₁ ∧ b.name = name) ∧  (b' ∈ l₂ ∧ b'.name = name) → ExponentOf name l₁ = b.exponent ∧ ExponentOf name l₂ = b'.exponent
  -- ∀ b ,b' : Base, (b ∈ l₁ ∧ b.name ≠ name) ∧ (b' ∈ l₂ ∧ b'.name ≠ name) → ExponentOf name l₁ = 0 ∧ ExponentOf name l₂ = 0
  by_cases h_b_l₁ : ∃ b : Base, (b ∈ l₁ ∧ b.name = name) <;> by_cases h_b_l₂ : ∃ b : Base, (b ∈ l₂ ∧ b.name = name)
  · obtain ⟨b, h_b_mem, h_b_name⟩ := h_b_l₁
    obtain ⟨b', h_b'_mem, h_b'_name⟩ := h_b_l₂
    have h_b_l₁' := exponentOf_eq_exp h₁ b h_b_mem
    have h_b_l₂' := exponentOf_eq_exp h₂ b' h_b'_mem
    rw [h_b_name] at h_b_l₁'
    rw [h_b'_name] at h_b_l₂'
    rw [h_b_l₁', h_b_l₂']
